<template>
  <div>
    ...
    <form
      ...
      class="my-form"
    >
      <label for="encadrantUniversitaire" class="label">Encadrant Universitaire:</label>
      <input
        type="number" 
        id="encadrantUniversitaire"
        v-model="formData.encadrent_universitaire_id"
        class="input"
      />
      ...
      <input
        type="text"
        id="encadrant"
        v-model="formData.encadrent_de_stage"
        class="input"
      />
...
const submitForm = async () => {
  ...
  try {
    const response = await fetch("http://localhost:3000/api/submit-pfe", {
      method: "POST",
